...
function LoginButton() {
    ...
    const signInSAML = async () => {
        ...
        const user_email = loginResult.additionalUserInfo.profile['urn:oid:0.9.2342.19200300.100.1.3']
    ...
    const login_logout_button = () => {
        ...
        let ll_button =
            <Button
                className="loginButton" variant="outlined" onClick={signInSAML}>Login</Button>
        if (user_email !== '' && user_email !== null) {
            ll_button =
                <Button
                    className="loginButton" variant="outlined" onClick={handleLogoutClick}>Logout: {user_email}
                </Button>
        }
        return ll_button
